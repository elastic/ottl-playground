...
func Test_AttributeProcessorExecutor_ParseConfig(t *testing.T) {
	yamlConfig, err := os.ReadFile(filepath.Join("..", "testdata", attributesprocessorConfig))
	require.NoError(t, err)

	executor := NewAttributesProcessorExecutor().(*attributesProcessorExecutor)
	assert.NotNil(t, executor)

	parsedConfig, err := executor.parseConfig(string(yamlConfig))
	require.NoError(t, err)
	require.NotNil(t, parsedConfig)
	require.NotEmpty(t, parsedConfig.Actions)
}
